...
| `document_bucket_cors_allow_origins` | `list(string)` | Origins allowed to presign-PUT to the bucket. Required (no default). Passed inline from `envs/dev/main.tf` as `["http://localhost:3000", local.site_origin]`, not a tfvars dial — `local.site_origin` now resolves to `module.cloudfront_origin.cloudfront_url`, so this list is "known after apply" on any plan that (re)creates the distribution |
