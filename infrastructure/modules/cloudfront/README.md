...
**Never let two `aws_s3_bucket_policy` resources target the site bucket in one apply.** A bucket has exactly one policy. Two resources at different addresses with no dependency edge between them apply in either order — and the plan shows nothing wrong. This module and `modules/s3Site` used to both declare one (`site_public_read` there, `attach_oac_control` here); migrating a *live* public bucket to OAC required stripping `s3Site`'s policy in its own apply first, before this module existed. **Resolved now, not just historically**: `s3Site`'s `site_public_read` resource is fully removed from its config (commented out in `s3.tf`, not just superseded in state), so `attach_oac_control` here is the bucket's only policy — a from-zero `apply` can never hit this race. Applied and verified: distribution `Status: Deployed`, `cloudfront_url` returns `200` on `/`, `/login`, and `/login/`, `404` on an unknown path.
...
**`terraform_data.site_deploy`'s `triggers_replace` doesn't watch `cloudfront_distribution_id`.** The `aws cloudfront create-invalidation` call is interpolated inside the `local-exec` command string, not listed as a trigger — only the bucket name and a hash of the frontend source files are. So creating this distribution for the first time does not itself rebuild/resync/invalidate; the bucket serves whatever was last synced. The first real frontend edit after this module applies is what fires the sync + invalidation using the real distribution ID.
